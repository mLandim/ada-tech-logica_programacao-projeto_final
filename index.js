...
// Utilitários
const validaTelefone = (telefoneRecebido) => {
...
const trataPrompt = (textoPrompt, permiteCancelar, valorDefault, validacaoFn, mensagemCancelamento) => {
    try {
        let textoCancelar = permiteCancelar ? '\n* Clique em "Cancelar" para sair' : '';
        let valorPrompt = prompt(`${textoPrompt}${textoCancelar}`, valorDefault);
        if (permiteCancelar && valorPrompt === null) {
            throw new Error(mensagemCancelamento !== null && mensagemCancelamento !== void 0 ? mensagemCancelamento : 'Operação cancelada');
        }
...
const idNaoLocalizado = (fnCallback, id, listaClientes) => {
...
const detalharCliente = (listaClientes, idRecebido) => {
    try {
        const textoAoresentacao = `Detalhar Cliente\n====================================\n\n`;
        const id = idRecebido !== null && idRecebido !== void 0 ? idRecebido : parseInt(trataPrompt(`${textoAoresentacao}Informe o ID do cliente que deseja exibir:`, true));
        ...
        if (clienteExibir) {
            ...
            idNaoLocalizado(detalharCliente, id, listaClientes);
            2;
        }
    }
    catch (error) {
...
const atualizarCliente = (listaClientes, idRecebido) => {
    try {
        const textoAoresentacao = `Atualizar Cliente\n====================================\n\n`;
        const id = idRecebido !== null && idRecebido !== void 0 ? idRecebido : parseInt(trataPrompt(`${textoAoresentacao}Informe o ID do cliente que deseja atualizar:`, true));
...
const deletarCliente = (listaClientes, idRecebido) => {
    try {
        const textoAoresentacao = `Deletar Cliente\n====================================\n\n`;
        const id = idRecebido !== null && idRecebido !== void 0 ? idRecebido : parseInt(trataPrompt(`${textoAoresentacao}Informe o ID do cliente que deseja deletar:`, true));
